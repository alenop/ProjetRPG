...
import rpg.eventhandler.GameOverHandler;
...
public abstract class DisplayCombat extends DisplayBasic {
	...
	public static void begin(Monstre monstre, Point2D posMonstre) {
		...
		listeBouton[1] = new Button("d�fense");
	...
	public static void mode_combat2(Entity viewcombat, Monstre monstre, Point2D posMonstre, int nb_tour) {
		String text;
		if (nb_tour == 1) {
			text = "Tu as trouv� " + monstre.getName() + " que veut tu faire ?";
		} else if (nb_tour == 2) {
			text = "Attention " + monstre.getName() + " pr�pare une grosse attaque";
		} else {
			text = monstre.getName() + " est encore vivant que veut tu faire ?";
		}
		if (nb_tour == 0) {
			text = monstre.getName() + " est mort !\nBravo tu as gagn� " + monstre.getGive_experience()
					+ " points d'exp�rience";
		...
		for (Node i : viewcombat.getView().getNodes()) {
			if (i.getAccessibleText() != null) {
				...
				else if (i.getAccessibleText().equals("d�fense")) {
					if (nb_tour==1) {
						((Button) ((Entity) i.getUserData()).getPropertyOptional("bouton").get()).setText("d�fense");
					}
					if (nb_tour==-1) {
						...
						.setOnAction(new CombatEventHandler(monstre, nb_tour, viewcombat, posMonstre,"d�fense"));
					}
				}
